...
class Normal_Loader(Dataset):
    ...
    """
    def __init__(self, is_train=1, path='/content/UCF-Crime/Real-world-Anomaly-Detection-in-Surveillance-Videos-pytorch/',path2="/content/UCF-Crime/"):
    ...
    def __getitem__(self, idx):
        if self.is_train == 1:
            rgb_npy = np.load(os.path.join(self.path2+'all_rgbs', self.data_list[idx][:-1]+'.npy'))
            flow_npy = np.load(os.path.join(self.path2+'all_flows', self.data_list[idx][:-1]+'.npy'))
            ...
            rgb_npy = np.load(os.path.join(self.path2+'all_rgbs', name + '.npy'))
            flow_npy = np.load(os.path.join(self.path2+'all_flows', name + '.npy'))
            concat_npy = np.concatenate([rgb_npy, flow_npy], axis=1)
